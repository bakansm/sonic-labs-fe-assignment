...
function TooltipContent({ className, sideOffset = 0, children, ...props }: ComponentProps<typeof Content>) {
  return (
    <Portal>
      <Content
        className={cn(
          'fade-in-0 zoom-in-95 z-50 w-fit origin-(--radix-tooltip-content-transform-origin) animate-in text-balance rounded-xs bg-foreground px-3 py-1.5 text-background text-body-xs',
